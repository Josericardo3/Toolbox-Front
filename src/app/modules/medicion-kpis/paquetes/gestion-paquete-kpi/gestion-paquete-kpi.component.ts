...
@Component({
  selector: 'app-gestion-paquete-kpi',
  templateUrl: './gestion-paquete-kpi.component.html',
  styleUrls: ['./gestion-paquete-kpi.component.css'],
})
export class GestionPaqueteKpiComponent {
  ...
  respuesta: any = {};
  constructor(
    private _dialog: MatDialogRef<GestionPaqueteKpiComponent>,
  ...
  registrar() {
    this.paqueteService.agregar(this.model).subscribe({
      next: (x) => {
        if (x.Confirmacion) {
  ...
  actualizar() {
    this.paqueteService.actualizar(this.model).subscribe({
      next: (x) => {
        if (x.Confirmacion) {
          this.respuesta.opcion = 1;
          this.respuesta.tipo = true;
